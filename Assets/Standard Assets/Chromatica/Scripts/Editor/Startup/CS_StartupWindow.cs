...
public class Chromatica_StartupWindowProcessor : AssetPostprocessor
{
	static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		string[] entries = System.Array.FindAll(importedAssets, name => name.Contains("CS_StartupWindow") && !name.EndsWith(".meta"));

		for (int i = 0; i < entries.Length; i++)
			if (Chromatica_StartupWindow.Init(false))
				break;
	}
}

public class Chromatica_StartupWindow : EditorWindow
{
	public static string identifier = "TH_Chromatica";
	static string pathChangelog = "Assets/Chromatica/Changelog.txt";
	static string pathImages = "Assets/Chromatica/Internal/Textures/";
	...
	[MenuItem("Help/Chromatica/About", false, 0)]
	public static void MenuInit()
	{
		Chromatica_StartupWindow.Init(true);
	}

	[MenuItem("Help/Chromatica/User Manual", false, 0)]
	public static void MenuManual()
	{
		Application.OpenURL("http://thomashourdel.com/chromatica/doc/");
	}

	public static void FindAssets()
	{
		// Get the relative data path
		string[] results = AssetDatabase.FindAssets("CS_StartupWindow t:Script", null);
		if (results.Length > 0)
		{
			...
			pathImages = p + "/Internal/Textures/";
		}
	}
	...
	public static bool Init(bool forceOpen)
	{
		...
		{
			Chromatica_StartupWindow window;
			window = EditorWindow.GetWindow<Chromatica_StartupWindow>(true, "About Chromatica", true);
	...
	void OnEnable()
	{
		...
		headerPic = LoadAssetAt<Texture2D>(pathImages + "CS_About_Header.jpg");
		iconColorful = LoadAssetAt<Texture2D>(pathImages + "CS_About_Icon_Colorful.png");
		iconChromatica = LoadAssetAt<Texture2D>(pathImages + "CS_About_Icon_Chromatica.png");
		iconSSAOPro = LoadAssetAt<Texture2D>(pathImages + "CS_About_Icon_SSAOPro.png");
		iconLutify = LoadAssetAt<Texture2D>(pathImages + "CS_About_Icon_Lutify.png");
	}
		...
		GUILayout.BeginVertical();
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				if (GUILayout.Button("<b>Support Forum</b>\n<size=9>Unity Community</size>", richButtonStyle, GUILayout.MaxWidth(172), GUILayout.Height(36)))
					Application.OpenURL("http://forum.unity3d.com/threads/chromatica-studio-the-definitive-color-grading-suite-for-unity-easy-fast-powerful.262489/");
			}
			GUILayout.EndHorizontal();
